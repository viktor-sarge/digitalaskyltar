...
class PlainBlogView(ItemView):
    """Contain a list of blog items without location and display detailed 
    ...
    def __init__(self, frame, bgcolor, settings):
        ...
        t = tki.Text(frame, width = width - (2 * textpadding), height = 96, wrap = tki.WORD, bg="white", font = self._fntNormal, 
                     relief = tki.FLAT)
        t.grid(row = 2, column = 0, padx = textpadding, pady = defaultpadding, sticky = tki.W + tki.N)
        self._tDesc = t
        
        if(settings.scrollenabled):
            bsb.addScrollable(t)

class BlogView(PlainBlogView):
    """Contain a list of blog items including image and location and display detailed 
    information about one of them.
    ...
    def toHtml(self):
        """Return an HTML representation of the currently displayed blog post"""
    ...
    def _displayItem(self, item):
        ...
        Arguments
        item -- the blog item to display
        
        """
        PlainBlogView._displayItem(self, item)
