...
from varbergsettings import Settings4, InifileDataSourceDescription, sBlog
import scheduler
from blogview import PlainBlogView
from varbergdatamanager import DataManager4Blog
...
class Controller:
    ...
    def __init__(self, inidir, inifile):
        ...
        """
        settings = Settings4(inidir, inifile)
        ...
        dm = DataManager4Blog(dsdblog, settings)
        self._datamanagers[_blogid] = dm
        self._sched.addUpdatable(dm)
        gui.setupSoftbutton(self._ehBlog, lang[lng.txtBlog], 1, dm)
        
        #Create book list view
        frame = gui.createMainAreaShape(publicdisplay.shpRoundRect, _blogid)
        blogview = BooklistView(settings.previewx, settings.previewy, frame)
        self._views[_blogid] = blogview
        #dm.addObserver(blogview)
